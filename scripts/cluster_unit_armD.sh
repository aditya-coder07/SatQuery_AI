#!/usr/bin/env bash
# Grounding arm D: failure-directed retraining. The best adapter so far
...
run train_ground_hard $PY training/train_vlm_sft.py --model $BASE \
  ...
  --ckpt-dir checkpoints/v3/grounding_vlm_hard --epochs 1 --batch-size 4 --grad-accum 4 --lr 3e-5 \
...
[ -f checkpoints/v3/grounding_vlm_hard/adapter_best/adapter_model.safetensors ] || { log "STOPPED: arm D adapter_best missing"; exit 1; }
run eval_ground_armD $PY evaluation/grounding_official_eval.py --base $BASE --data data/dior_rsvg_official \
  --arms init=$INIT_ADAPTER lora_hard=checkpoints/v3/grounding_vlm_hard/adapter_best \
  --out artifacts/benchmark_reports/dior_rsvg_official_armD.json --batch 16
