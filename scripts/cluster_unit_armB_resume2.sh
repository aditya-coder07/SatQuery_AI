...
LOG=logs/queue_armB_resume2.log
MIN_FREE_GB=${MIN_FREE_GB:-14}
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
source scripts/cluster_unit_lib.sh
log "arm B unit started"
run train_ground_lora_merger $PY training/train_grounding_vlm.py --model $BASE --data data/dior_rsvg_official   --ckpt-dir checkpoints/v3/grounding_vlm_r16_merger --epochs 1 --batch-size 2 --grad-accum 8 --lr 1e-4   --val-limit 400 --val-every 800 --save-every 400 --workers 6 --quant none --resume --train-merger   --notes "arm B: LoRA r16 + trainable visual merger, official DIOR-RSVG train, 1 epoch, batch 2x8"
