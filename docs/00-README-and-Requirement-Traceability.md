...
| **I2** | Co-registered optical + SAR **pair** | `config = CROSSMODAL_PAIR` is detected correctly. **The compatibility gates are declared and not enforced** — see **L16** | `test_ingest.py`, `test_real_products.py` | **PARTIAL** — the configuration is recognised and the pair is analysed, but an optical+SAR pair with *disjoint footprints* is accepted and fused without a failing check. The PS's word is **"co-registered"**, and nothing verifies it |
| **I3** | **Bi-temporal** pair of the same area | `config = BITEMPORAL_PAIR`; missing dates are recorded as a **WARN** (`temporal_order`: order taken from input order) rather than an abstention, and the matrix's `require_dates: true` is not read — see **L16** | `test_ingest.py`, `test_abstention.py` (25) | **PARTIAL** — configuration and ordering work; "same area" is unverified for the same reason as I2 |
