...
| grounding | LoRA r16 adapter (arm A) | v2 CNN (0.160, Category C) | **select arm C** (`grounding_vlm_vrs/adapter_best`): DIOR-RSVG 0.6880 = arm A (n.s.), VRSBench val 0.6325 vs 0.501; arm B rejected; unified rejected (0.638); arm D (hard negatives from A) 0.6992 significant → arm D′ (same recipe from C) running and will replace C if it holds both numbers | official test 0.6877 [0.677, 0.699] vs zero-shot 0.3823, McNemar p ≪ 0.001; 4-bit deployed path 0.678 (−1.0 pt, parity accepted) |
