...
| grounding | LoRA r16 adapter (arm A) | v2 CNN (0.160, Category C) | **select arm E** (`grounding_vlm_hires/adapter_best`, served with `SATQUERY_GROUNDING_MIN_PIXELS=1048576`): DIOR-RSVG **0.7323** (D′ 0.7020, McNemar χ² 47) and VRSBench val **0.6594** (D′ 0.6396; clean 0.6237) — holds both; lineage A → C (+VRSBench) → D′ (+hard negatives) → E (+resolution) | official test 0.6877 [0.677, 0.699] vs zero-shot 0.3823, McNemar p ≪ 0.001; 4-bit deployed path 0.678 (−1.0 pt, parity accepted) |
...
| landcover (class-balanced arm, 2026-09-18) | `landcover_full_balanced/best.pt` (pos_weight ≤ 5, noise aug) | `landcover_full` | **rejected** | official test macro 0.7879 / micro 0.8726 vs 0.7923 / 0.8850; 17/19 classes down, rarest two up (+0.09, +0.02); rule was macro up AND micro not worse — neither holds; deployment map unchanged |
| unified adapter | one LoRA over 4 tasks | — | **rejected for every task** (−5.2 / −0.7 / −2.0 / −1.0 pts vs specialists on the official tests); kept as single-adapter fallback | `docs/research/ablations.md` §Unified |
...
| grounding (7B arm, #26, 2026-09-19) | `grounding_7b_hires/adapter_best` (Qwen2.5-VL-7B QLoRA r16, same data as arm E, 1024²) | arm E (3B) 0.7323 | **not selected** | official test 0.7392 vs 0.7323, McNemar χ² 2.29 n.s. (rule 1 not met); 2.3× weights, ≈ 2× latency, would break the one-shared-base layout (rule 5) |
| change_mask (TTA + retrain arms, 2026-09-21) | champion + 8-fold dihedral TTA; `change_mask_boundary/best.pt`; `change_mask_cont/best.pt` | v3 champion 0.9038 | **select TTA, reject both retrains** | official test: TTA 0.9101 at 0.5 / 0.9139 at val-thr 0.75 (CI [0.906, 0.914] excludes 0.9038); boundary fine-tune 0.8941 (val 0.9006 < champion 0.9055); plain continuation 0.9022 — weights unchanged, tool 1.1.0 serves TTA (`SATQUERY_CHANGE_MASK_TTA=0` restores the plain pass) |
