...
| grounding | LoRA r16 adapter (arm A) | v2 CNN (0.160, Category C) | **select arm D′** (`grounding_vlm_hard_vrs/adapter_best`): DIOR-RSVG **0.7020** (arm C 0.688, McNemar significant) and VRSBench val **0.6396** (arm C 0.6325) — holds both; lineage A → C (+VRSBench) → D′ (+hard negatives); arm B and unified rejected | official test 0.6877 [0.677, 0.699] vs zero-shot 0.3823, McNemar p ≪ 0.001; 4-bit deployed path 0.678 (−1.0 pt, parity accepted) |
