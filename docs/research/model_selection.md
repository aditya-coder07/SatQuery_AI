...
| grounding | LoRA r16 adapter (arm A) | v2 CNN (0.160, Category C) | **select** (arms B/C may replace) | official test 0.6877 [0.677, 0.699] vs zero-shot 0.3823, McNemar p ≪ 0.001; 4-bit deployed path 0.678 (−1.0 pt, parity accepted) |
...
| change_caption | VLM two-image adapter (`change_caption_vlm/adapter_best`, step 2,000 val-selected of 2,129 — the last 89 steps were lost to the 2026-09-14 reboot, lr < 1e-6 by then) | v1 (0.384, oracle-mask) | official test running | VLM arm sees images only — the fair comparison |
