...
| `sq-unit-unified-resume3` evals | all four DONE (exit 0) by 03:24 | `checkpoints/v3/unified_vlm/adapter_best` (backed up tier 1) | `dior_rsvg_official_unified.json` 0.6379, `rsvqa_lr_official_unified.json` 0.9053, `rsicd_test_unified.json` BLEU-4 0.2349, `levircc_test_unified.json` 0.5949 — specialists win every task | `eval_*` records |
| `sq-unit-armD` training | DONE 04:15 (exit 0); best val Acc@0.5 **0.72** | `checkpoints/v3/grounding_vlm_hard/adapter_best` | official test running (init vs lora_hard, paired) | `vlm_sft-…` done |
| `sq-unit-vrsbench-eval` | DONE 01:27 (exit 0) | read-only | `vrsbench_val_grounding.json` n 16,159 × 2 arms: lora_vrs 0.6325 vs lora_r16 0.5011; `_subsets.json` clean 0.5863 vs 0.4463 | `eval_grounding_*` |
