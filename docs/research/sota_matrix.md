...
| DIOR-RSVG official test | Acc@0.5 | 0.160 **(C — trained on test split, self-made subset)** | **0.6877** LoRA r16 arm A [0.677, 0.699] (zero-shot 0.3823; image-disjoint subset 0.805); arms B/C queued | LQVG 83.4; LPVA 82.8; GeoGround 77.7; EarthGPT 76.7 | −15 (arm A) | **A** for Phase 6 | image-disjoint subset 0.4764 |
...
| Landsat-SCD test (CC BY 4.0; self-made 3:1:1 split by original, 477 test pairs) | 10-way change-type mIoU / SeK / Score = 0.3 mIoU + 0.7 SeK; binary change F1 | none | **0.573 / 0.460 / 0.504**; binary F1 0.851, OA 0.929 (siamese R50 + FPN, 40 ep, val-selected epoch 39) | no published number on this split; the Landsat-SCD paper decodes per-date maps and reports on its own split | n/a | **B** | replaces SECOND as the licensed semantic-change row; val still rising at epoch 39 → longer schedule is the next arm |
